...
class GRU(nn.Module):
    ...
    def forward(self, x):
        ...
        # Forward propagate through GRU using the input sequence
        out, _ = self.gru(x, h0)
        ...
        for _ in range(self.predict_length):
            ...
            # Update last hidden state using GRU (reshape to add batch and seq dimensions)
            last_hidden, _ = self.gru(input.unsqueeze(1), h0)
